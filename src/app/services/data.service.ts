...
export class DataService {
  ...
  private popUpThread = new Subject<Popup>();
  ...
  togglePopUp(popUp: Popup) {
    this.popUpThread.next(popUp);
  }
  listenPopUp(): Observable<Popup> {
    return this.popUpThread.asObservable();
  }
  ...
  getForecastByPositions(positions: Position): Observable<any> {
    const request = new HttpParams()
      ...
      .set('lat', positions.coords.latitude.toString())
      .set('lon', positions.coords.longitude.toString());
    return this.http.get(environment.BASE_FORECAST_URL, {params: request}).pluck('list');
  }
}
